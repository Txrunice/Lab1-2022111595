        ...
        switch (choice) {
            ...
            case 5: {
                ...
                if (!startWord.empty()) {
                    ...
                    // 如果找到了路径，询问是否要导出路径图
                    if (pathResult.find("不存在路径") == std::string::npos && !endWord.empty()) {
                        std::cout << "是否要将最短路径可视化？(y/n): ";
                        ...
                        if (visualize == "y" || visualize == "Y") {
                            // 解析路径结果，提取所有路径
                            std::vector<std::vector<std::string>> allPaths;
                            // 这里简化处理，假设已经有函数能从calcShortestPath返回中提取路径
                            // 实际实现中需要解析pathResult或修改calcShortestPath返回路径数据结构
                            
                            // 导出路径到DOT文件
                            std::string dotFilePath;
                            std::cout << "请输入DOT文件的保存路径（默认为path_{source}_to_{target}.dot）: ";
                            std::getline(std::cin, dotFilePath);
                            
                            // 假设这里能获取到最短路径
                            // textGraph.exportPathToDOT(startWord, endWord, allPaths, dotFilePath);
                            ...
                            if (generateImage == "y" || generateImage == "Y") {
                                ...
                                std::cout << "请输入图像文件的保存路径（默认为path_{source}_to_{target}.png）: ";
                                std::getline(std::cin, imageFilePath);
                                // textGraph.generateGraphImage(dotFilePath, imageFilePath);
                            }
                        }
